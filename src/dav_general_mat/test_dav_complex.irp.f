...
subroutine dav_complex
 implicit none
 complex*16, allocatable :: u_in(:,:), H_jj(:), energies(:)
 ...
 call diag_general_complex(eigvalues,eigvectors,H_matrix_all_dets_complex,sze,sze)

 eigvalues += dcmplx(nuclear_repulsion,0d0)
 print*,'Exact energies:'
 do i = 1, N_st
    write(*,'(I6,2(F18.10))') i, REALPART(eigvalues(i)), IMAGPART(eigvalues(i))
 enddo
