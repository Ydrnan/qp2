...
&BEGIN_PROVIDER [double precision, mo_wz_cap , (mo_num,mo_num)]
 BEGIN_DOC
 ! array of the integrals of MO_i * W_x MO_j
 ! array of the integrals of MO_i * W_y MO_j
 ! array of the integrals of MO_i * W_z MO_j
 END_DOC
 implicit none
...
END_DOC
 implicit none
  ...
  !$OMP SHARED(nucl_coord,ao_power,ao_prim_num,center_of_mass,cap_box_size, &
  !$OMP  ao_wx_cap,ao_wy_cap,ao_wz_cap,ao_num,ao_coef_normalized_ordered_transp,ao_nucl, &
   ...
   do i= 1,ao_num
    ...
    do n = 1,ao_prim_num(j)
     ...
     do l = 1, ao_prim_num(i)
      ...
      call overlap_bourrin_spread_cap(A_center(1),B_center(1),alpha,beta,power_A(1),power_B(1),tmp,lower_exp_val,dx,dim1,center_of_mass(1),cap_box_size(1))
      !print*,'ici'
      !print*,i,j,n,l
      !print*,'c',ao_coef_normalized_ordered_transp(n,j),ao_coef_normalized_ordered_transp(l,i),'a',alpha,'b',beta
      !print*,'int',c*tmp
      !print*,'res', c*tmp*overlap_y*overlap_z  
      accu_x +=  c*tmp*overlap_y*overlap_z
      call overlap_bourrin_spread_cap(A_center(2),B_center(2),alpha,beta,power_A(2),power_B(2),tmp,lower_exp_val,dx,dim1,center_of_mass(2),cap_box_size(2))
      accu_y +=  c*tmp*overlap_x*overlap_z
      call overlap_bourrin_spread_cap(A_center(3),B_center(3),alpha,beta,power_A(3),power_B(3),tmp,lower_exp_val,dx,dim1,center_of_mass(3),cap_box_size(3))
...
subroutine overlap_bourrin_spread_cap(A_center,B_center,alpha,beta,power_A,power_B,overlap_x,lower_exp_val,dx,nx,barycenter_x,cap_x)
  ...
  !  int [-infty ; +infty] of [(x-A_center)^(power_A) * (x-B_center)^power_B * exp(-alpha(x-A_center)^2) * exp(-beta(x-B_center)^2) * (x - x_center) if  abs(x) > x_center ]
   ...
   do i= 1,ao_num
    ...
    do n = 1,ao_prim_num(j)
     ...
     do l = 1, ao_prim_num(i)
      ...
      !print*,i,j,n,l
      !print*,'c',ao_coef_normalized_ordered_transp(n,j),ao_coef_normalized_ordered_transp(l,i),'a',alpha,'b',beta
      !print*,'int',c*(overlap_x2 + 2d0 * A_center(1) * overlap_x1 + A_center(1)**2 * overlap_x0 )
      !print*,'over',overlap_x0,overlap_y0,overlap_z0,power_A,power_B
      !print*,'res',c*(overlap_x2 + 2d0 * A_center(1) * overlap_x1 + A_center(1)**2 * overlap_x0 )*overlap_y0 * overlap_z0
